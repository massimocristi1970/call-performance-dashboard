// js/renderers.js - Clean Fixed Version
...
class PageRenderer {
  ...
  async renderPage(pageKey, containerId){
    ...
    if (pageKey === 'fcr') {
      ...
      // Chart 2: Call Volume Comparison
    ...
    if (pageKey === 'outbound') {
      ...
      outboundCallsData.forEach(r => {
        ...
        if (!a) return;
        byAgent[a] = (byAgent[a] || 0) + cleanNumber(r['Outbound Calls']);
      });
      ...
      // Chart 4: Average Calls Per Hour by Agent
      ...
      const outboundAgentAvgData = Object.entries(outboundCallsPerHourByAgent)
        .map(([agent, data]) => ({
          agent,
          avgCallsPerHour: data.days.size > 0 ? data.calls / (data.days.size * 8) : 0
        }))
  ...
  calculateKPIs(pageKey, data) {
    ...
    if (pageKey === 'outbound') {
      ...
      out.totalCalls = totalOutboundCalls;
      out.connectRate = actualConnectRate;
